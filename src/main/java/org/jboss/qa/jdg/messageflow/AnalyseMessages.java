...
public class AnalyseMessages implements Processor {
   ...
   private static class TaggedStats {
      ...
      public Map<String, Average> handleDelay = new TreeMap<String, Average>();
      ...
      public boolean equals(Object o) {
         ...
         return mapEquals(handleDelay, that.handleDelay) && mapEquals(routeStats, that.routeStats);
      }
      ...
      for (String message : trace.messages) {
         ...
         for (Event event : trace.events) {
            ...
            if (event.type == Event.Type.OUTCOMING_DATA_STARTED || event.type == Event.Type.RETRANSMISSION) {
               sent.add(event);
            } else if (event.type == Event.Type.HANDLING || event.type == Event.Type.DISCARD) {
   ...
   public void finish() {
      ...
      for (TaggedStats stats : this.stats.values()) {
         ...
         out.println("\tIncoming to handle times:");
         for (Map.Entry<String, Average> entry : stats.handleDelay.entrySet()) {
